...
def load_nips_data():
  ...
  word_idx = np.logical_and(np.sum(x_full != 0, 1) >= 2,
                            np.sum(x_full, 1) >= 10)
...
def train_and_test(architecture, savedir, skip_connections=False, q='lognormal', lr=0.01, n_epoch=1, 
                   n_iter_per_epoch=40000, cv=False, batch_size=-1, truncate=True, seed=42, n_test_epoch=50, 
                   n_test_iter=500, map_estimate=False, data='nips', most_skip=False):
  ...
  with sess.as_default():
    ...
    train_inds = np.random.choice(len(x_full), (len(x_full) - 1000), replace=False)
    ...
    for i in range(len(K) - 1, 0, -1):
      scale_denominator = tf.matmul(zs['z' + str(i + 1)], Ws['W' + str(i)])
      if skip_connections & (i % 3 == 1):
        scale_denominator = scale_denominator + zs['z' + str(i + 2)]
      if most_skip:
          if i % 3 == 1:
            scale_denominator = scale_denominator + zs['z' + str(i + 2)] + zs['z' + str(i + 1)]
          if i % 3 == 2:
            scale_denominator = scale_denominator + zs['z' + str(i + 1)]
    ...
    for i in range(1,len(K) + 1):
      if map_estimate:
        qW_variables[str(i-1)] = tf.Variable(tf.random_normal(Ws['W' + str(i-1)].shape), name='W'+str(i))
        qWs['qW' + str(i-1)]= PointMass(tf.maximum(tf.nn.softplus(qW_variables[str(i-1)]), min_mean))
        if q == 'lognormal':
          qz_variables['loc' + str(i)] = tf.Variable(tf.random_normal([N, K[i-1]]), name='loc_z' + str(i))
          qz_variables['scale' + str(i)] = tf.Variable(0.1 * tf.random_normal([N, K[i-1]]), name='scale_z' + str(i))
          if M != -1: # TO DO: batch size doesn't matter for W
            qzs['qz' + str(i)] = TransformedDistribution(
                distribution=Normal(
                    tf.gather(qz_variables['loc' + str(i)], idx_ph),
                    tf.maximum(tf.nn.softplus(tf.gather(qz_variables['scale' + str(i)], idx_ph)), min_scale)),
                bijector=tf.contrib.distributions.bijectors.Exp())
          else:
            qzs['qz' + str(i)] = TransformedDistribution(
                distribution=Normal(
                    qz_variables['loc' + str(i)],
                    tf.maximum(tf.nn.softplus(qz_variables['scale' + str(i)]), min_scale)),
                bijector=tf.contrib.distributions.bijectors.Exp())
        else:
          ...
          if M != -1:
            ...
            qzs['qz' + str(i)] = Gamma(tf.maximum(tf.nn.softplus(qz_variables['shape' + str(i)]), min_shape),
                  tf.maximum(1.0 / tf.nn.softplus(qz_variables['scale' + str(i)]), 1.0 / min_scale))
      else:
        if q == 'lognormal':
          qW_variables['loc' + str(i-1)] = tf.Variable(tf.random_normal(Ws['W' + str(i-1)].shape), name='loc_w' + str(i-1))
          qW_variables['scale' + str(i-1)] = tf.Variable(0.1 * tf.random_normal(Ws['W' + str(i-1)].shape), name='scale_w' + str(i-1))
          qz_variables['loc' + str(i)] = tf.Variable(tf.random_normal([N, K[i-1]]), name='loc_z' + str(i))
          qz_variables['scale' + str(i)] = tf.Variable(0.1 * tf.random_normal([N, K[i-1]]), name='scale_z' + str(i))
          if M != -1: # TO DO: batch size doesn't matter for W
            qWs['qW' + str(i-1)] = TransformedDistribution(
                distribution=Normal(
                    ...
                    tf.maximum(tf.nn.softplus(qW_variables['scale' + str(i-1)]), min_scale)),
                bijector=tf.contrib.distributions.bijectors.Exp())
            qzs['qz' + str(i)] = TransformedDistribution(
                distribution=Normal(
                    tf.gather(qz_variables['loc' + str(i)], idx_ph),
          ...
          qW_variables['shape' + str(i-1)] = tf.Variable(0.5 + 0.1 * tf.random_normal(Ws['W' + str(i-1)].shape))
          qW_variables['scale' + str(i-1)] = tf.Variable(0.1 * tf.random_normal(Ws['W' + str(i-1)].shape))
          qz_variables['shape' + str(i)] = tf.Variable(0.5 + 0.1 * tf.random_normal([N, K[i-1]]))
          qz_variables['scale' + str(i)] = tf.Variable(0.1 * tf.random_normal([N, K[i-1]]))
          if M != -1:
            qWs['qW' + str(i-1)] = Gamma(tf.maximum(tf.nn.softplus(qW_variables['shape' + str(i-1)]), min_shape),
                  tf.maximum(1.0 / tf.nn.softplus(qW_variables['scale' + str(i-1)]), 1.0 / min_scale))
            qzs['qz' + str(i)] = Gamma(tf.maximum(tf.nn.softplus(tf.gather(qz_variables['shape' + str(i)], idx_ph)), min_shape),
                  tf.maximum(1.0 / tf.nn.softplus(tf.gather(qz_variables['scale' + str(i)], idx_ph)), 1.0 / min_scale))
          else:
            qWs['qW' + str(i-1)] = Gamma(tf.maximum(tf.nn.softplus(qW_variables['shape' + str(i-1)]), min_shape),
    ...
    for i in range(1, len(K)):
      Ws_test['W' + str(i)] = Gamma(0.1, 0.3, sample_shape=[K[i], K[i-1]])

    zs_test = {}
    zs_test['z' + str(len(K))] = Gamma(0.1, 0.1, sample_shape=[N_test, K[-1]])
    for i in range(len(K) - 1, 0, -1):
      scale_denominator_test = tf.matmul(zs_test['z' + str(i + 1)], Ws_test['W' + str(i)])
      if skip_connections & (i % 3 == 1):
        scale_denominator_test = scale_denominator_test + zs_test['z' + str(i + 2)]
      if most_skip:
        if i % 3 == 1:
          scale_denominator_test = scale_denominator_test + zs_test['z' + str(i + 2)] + zs_test['z' + str(i + 1)]
        if i % 3 == 2:
          scale_denominator_test = scale_denominator_test + zs_test['z' + str(i + 1)]
      zs_test['z' + str(i)] = Gamma(shape, shape / scale_denominator_test)
    ...
    for i in range(1,len(K) + 1):
      if q == 'lognormal':
        qz_variables_test['loc' + str(i)] = tf.Variable(tf.random_normal([N_test, K[i-1]]))
        qz_variables_test['scale' + str(i)] = tf.Variable(0.1 * tf.random_normal([N_test, K[i-1]]))
        qzs_test['qz' + str(i)] = TransformedDistribution(
              distribution=Normal(
                  qz_variables_test['loc' + str(i)],
                  tf.maximum(tf.nn.softplus(qz_variables_test['scale' + str(i)]), min_scale)),
              bijector=tf.contrib.distributions.bijectors.Exp())
      else:
        qz_variables_test['shape' + str(i)] = tf.Variable(0.5 + 0.1 * tf.random_normal([N_test, K[i-1]]))
        qz_variables_test['scale' + str(i)] = tf.Variable(0.1 * tf.random_normal([N_test, K[i-1]]))
        qzs_test['qz' + str(i)] = Gamma(tf.maximum(tf.nn.softplus(qz_variables_test['shape' + str(i)]), min_shape),
                tf.maximum(1.0 / tf.nn.softplus(qz_variables_test['scale' + str(i)]), 1.0 / min_scale))
    ...
    for i in range(len(K)):
      inference_z_map_test[zs_test['z' + str(i + 1)]] = qzs_test['qz' + str(i + 1)]
      inference_z_data_test[Ws_test['W' + str(i)]] = qWs['qW' + str(i)]
    inference_z_test = ed.KLqp(inference_z_map_test, data=inference_z_data_test)

    optimizer_z_test = tf.train.AdamOptimizer(lr)
    inference_z_test.initialize(n_print=100,
    ...
    for epoch in range(n_epoch):
      ...
      for t in range(1, n_iter_per_epoch + 1):
        ...
        if M != -1:
          ...
          if map_estimate:
            info_dict = inference_z.update(feed_dict={x_ph: x_batch, idx_ph: idx_batch})
            info_dict_m = inference_w.update(feed_dict={x_ph: x_batch, idx_ph: idx_batch})
          else:
            info_dict = inference_func.update(feed_dict={x_ph: x_batch, idx_ph: idx_batch})
        else:
          if map_estimate:
            info_dict = inference_z.update()
            info_dict_m = inference_w.update()
          else:
      ...
      if map_estimate:
        ...
        if q == 'gamma':
          ...
          qW0_vals = sess.run(tf.exp(qWs['qW0'].distribution.mean() + qWs['qW0'].distribution.variance()/2))
      ...
      for epoch_test in range(n_test_epoch):
        ...
        if q == 'gamma':
          ...
          z1_mean = sess.run(tf.exp(qzs_test['qz1'].distribution.mean() + qzs_test['qz1'].distribution.variance()/2))#sess.run(qzs_test['qz1'].mean())
        w0_mean = qW0_vals

        doc_rate = np.matmul(z1_mean, w0_mean) # I think the issue is there's a Nan here
        doc_rate_normalized = doc_rate / np.sum(doc_rate, axis=1).reshape(len(doc_rate), 1)
        test_perp = np.exp(-1 * np.sum(np.log(doc_rate_normalized) * x_test_remaining)/np.sum(x_test_remaining))
        test_perplexities.append(test_perp)

        summary = sess.run(write_op, {perplexity_var: test_perp})
        if not map_estimate:
          inference_func.train_writer.add_summary(summary, epoch * n_test_epoch + epoch_test)
        else:
          inference_z.train_writer.add_summary(summary, epoch * n_test_epoch + epoch_test)
    ...
    for i in range(len(K)):
      if map_estimate:
        ...
        if q == 'gamma':
          ...
          final_zs[i+1] = sess.run(tf.exp(qzs['qz' + str(i + 1)].distribution.mean() + qzs['qz' + str(i + 1)].distribution.variance()/2))
      else:
        if q == 'gamma':
          final_zs[i+1] = sess.run(qzs['qz' + str(i+1)].mean())
          ...
          final_zs[i+1] = sess.run(tf.exp(qzs['qz' + str(i + 1)].distribution.mean() + qzs['qz' + str(i + 1)].distribution.variance()/2))
          final_Ws[i] = sess.run(tf.exp(qWs['qW' + str(i)].distribution.mean() + qWs['qW' + str(i)].distribution.variance()/2))
      np.save(savedir + '/z' + str(i + 1), final_zs[i+1])
      np.save(savedir + '/W' + str(i), final_Ws[i])
    np.save(savedir + '/test_perplexities', np.array(test_perplexities))
    np.save(savedir + '/losses', np.array(losses))

    tf.reset_default_graph()

    return np.array(test_perplexities), np.array(losses), final_zs, final_Ws

def wrapper(architecture, savedir, skip_connections=True, q='lognormal', lr=0.01, n_epoch=1, 
                   n_iter_per_epoch=40000, cv=False, batch_size=-1, truncate=True, seed=42, n_test_epoch=50, 
                   n_test_iter=500):
  sess = tf.InteractiveSession()
  with sess.as_default():
    perps, losses, zs, ws = train_and_test(architecture, savedir, skip_connections=skip_connections, q=q, lr=lr, n_epoch=n_epoch,
                                      n_iter_per_epoch=n_iter_per_epoch, cv=cv, batch_size=batch_size,
                                      seed=seed, n_test_epoch=n_test_epoch, n_test_iter = n_test_iter)
  return perps, losses, zs, ws