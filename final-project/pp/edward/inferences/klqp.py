...
from pp.edward.inferences.variational_inference import VariationalInference
from pp.edward.models import RandomVariable
from pp.edward.util import copy, get_descendants

try:
  from pp.edward.models import Normal
...
class KLqp(VariationalInference):
  ...
  def initialize(self, n_samples=1, kl_scaling=None, control_variates=False, *args, **kwargs):
  ...
  for var in var_list:
    ...
    if inference.control_variates:
        ...
        for i in range(inference.n_samples):
          fs.append(tf.gradients(-qi_log_prob[i] * 
            tf.stop_gradient(pi_log_prob[i] - qi_log_prob[i]), var)[0])
          hs.append(tf.gradients(-qi_log_prob[i], var)[0])
        fs = tf.stack(fs) # n_samples by n_docs by layer_dim
        hs = tf.stack(hs) # n_samples by n_docs by layer_dim
      
        f_mu = tf.reduce_mean(fs, 0)
        h_mu = tf.reduce_mean(hs, 0)
        cv_coefs = tf.divide(tf.reduce_sum(tf.multiply(fs - f_mu, hs - h_mu),
            0), (inference.n_samples - 1))
        grad_unmeaned = tf.subtract(fs, tf.multiply(tf.expand_dims(cv_coefs, 0), hs))
        grads.append(tf.reduce_mean(grad_unmeaned, 0))
    else:
        #grad_unmeaned = fs
        grad = tf.gradients(
            -tf.reduce_mean(qi_log_prob *
        ...
        grads.extend(grad)
    #grad = tf.reduce_mean(grad_unmeaned, 0)
    #grads.append(grad)
    grads_vars.append(var)
    
    #variance = tf.reduce_mean(tf.square(tf.subtract(grad_unmeaned, 
    #    tf.expand_dims(tf.reduce_mean(grad_unmeaned, 0), 0))), 0)
    #tf.summary.histogram("variance/" + 
    #                        var.name.replace(':', '/'),
    #                        variance, collections=[inference._summary_key])
    #tf.summary.scalar("variance mean/" + 
    #                     var.name.replace(':', '.'), tf.reduce_mean(variance),
    #                     collections=[inference._summary_key])
